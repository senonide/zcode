...
pub const Parser = opaque {
    ...
    pub const Options = extern struct {
        ...
        ///
        /// LOCAL FIX (not upstream): the C API declares this as
        /// `bool (*)(TSParseState *)`, and taking `State` by value instead put
        /// the `TSParseState *` where the callback reads `payload`.  The
        /// callback then compared against whatever those bytes happened to be,
        /// so it never cancelled and the parse always ran to completion.
        progress_callback: *const fn (state: *const State) callconv(.c) bool,
    };
};
